...
import { ChevronRight, Smartphone, Zap, Wallet, Star } from "lucide-react";
...
const content: Record<string, { title: string; steps: { num: string; text: string }[] }> = {
  app: {
    ...
    steps: [
      ...
      { num: "٤", text: "استكشف الأقسام السبعة من الشاشة الرئيسية وابدأ نشاطك." },
    ],
  },
  activities: {
      ...
      <div className="flex gap-2 px-4 pt-4 pb-2 overflow-x-auto">
        {tabs.map((t) => (
          <button key={t.id} onClick={() => setActiveTab(t.id)}
            className={`flex items-center gap-1.5 px-4 py-2 rounded-2xl font-arabic text-sm whitespace-nowrap flex-shrink-0 transition-all active:scale-95 ${activeTab === t.id ? "bg-primary text-primary-foreground font-bold shadow-md" : "bg-card border border-border text-muted-foreground"}`}
            style={activeTab === t.id ? { boxShadow: "0 3px 0 rgba(0,0,0,0.3)" } : {}}>
            ...
            <div className="space-y-4">
              {tab.steps.map((s, i) => (
                <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.08 }}
                  ...
                  <div className="w-10 h-10 rounded-xl bg-primary/15 flex items-center justify-center flex-shrink-0 font-arabic font-bold text-primary text-lg">
                    {s.num}
                  </div>
